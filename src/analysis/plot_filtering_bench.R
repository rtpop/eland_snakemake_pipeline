...
option_list <- list(
    optparse::make_option(c("-i", "--input"), type = "character", help = "Comma-separated list of paths to the compiled benchmark data frames."),
    ...
    optparse::make_option(c("-u", "--include-unfiltered"), type = "logical", default = TRUE, help = "Include the 'Unfiltered' network in the plot.")
)
...
if ( is.null(opt$`output-dir`) || is.null(opt$`tissue-type`)) {
    optparse::print_help(opt_parser)
    stop("Please provide the required arguments: -o and -t.")
}
